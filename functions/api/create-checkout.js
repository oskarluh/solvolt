export async function onRequestPost(context) {
  try {
    ...
    form.set('custom_text[submit][message]', 'Tarne lepitakse pärast makset eraldi kokku. Kui ostad ettevõttele, lisa allpool firma nimi ja KMKR number, et saaksime väljastada ametliku arve.');
    ...
    // Append success_url manually — URLSearchParams encodes {} to %7B%7D which Stripe rejects
    const body_str = form.toString()
      + '&success_url=' + siteUrl + '/edasi.html?session_id={CHECKOUT_SESSION_ID}';
    ...
    if (!stripeRes.ok) {
      ...
      return json({ error: stripeData?.error?.message || 'Makse alustamine ebaõnnestus' }, 500);
    }

    return json({ url: stripeData.url }, 200);
  } catch (error) {
    console.error('Checkout function error:', error);
    return json({ error: 'Makse alustamine ebaõnnestus: ' + error.message }, 500);
  }
}
